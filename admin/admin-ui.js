...
$("#btn-validate-admin").click(function () {
  ...
  if (
    !adminUserName ||
    !adminPassword ||
    !adminUserName.replace(/ /g, "").length ||
    !adminPassword.replace(/ /g, "").length
  ) {
    alertBox(
...
function connectSocket(username, password) {
  socket = io.connect(
    "/?userid=admin&adminUserName=" + username + "&adminPassword=" + password
  );
...
function updateListOfUsers(listOfUsers) {
  ...
  if (!listOfUsers.length) {
    $("#users-list").html(
      "<tr><td colspan=8>No active user found on this server.</td></tr>"
    );
    return;
  }
  ...
  listOfUsers.forEach(function (user, idx) {
    ...
    html +=
      '<td><span class="clickable max-width" data-userid="' +
      user.userid +
      '" title="' +
      user.userid +
      '">' +
    ...
    html +=
      '<td><span class="max-width" title="' +
      JSON.stringify(user.admininfo.extra || {}).replace(/"/g, "`") +
      '">' +
      JSON.stringify(user.admininfo.extra) +
      "</span></td>";
    ...
    html +=
      '<td><span class="max-width" title="' +
      JSON.stringify(user.admininfo.mediaConstraints || {}).replace(/"/g, "`") +
      '">' +
      JSON.stringify(user.admininfo.mediaConstraints) +
      "</span></td>";
    html +=
      '<td><span class="max-width" title="' +
      JSON.stringify(user.admininfo.sdpConstraints || {}).replace(/"/g, "`") +
      '">' +
      JSON.stringify(user.admininfo.sdpConstraints) +
      "</span></td>";
    html +=
      '<td><span class="max-width" title="' +
      JSON.stringify(user.connectedWith || {}).replace(/"/g, "`") +
      '">' +
      JSON.stringify(user.connectedWith) +
      "</span></td>";

    html +=
      '<td><button class="btn delete-user" data-userid="' +
      user.userid +
      '">Delete</button></td>';
    $(tr).html(html);
    ...
    $(tr)
      ...
      .click(function () {
        ...
        confirmBox(
          'User "<b>' + userid + '</b>" will be deleted from server.',
...
function updateListOfRooms(rooms) {
  ...
  if (!keys.length) {
    $("#rooms-list").html(
      "<tr><td colspan=9>No active room found on this server.</td></tr>"
    );
    return;
  }
  ...
  keys.forEach(function (roomid, idx) {
    ...
    room.participants.forEach(function (pid) {
      html +=
        '<span class="clickable userinfo" data-userid="' +
        pid +
        '"><span class="max-width" title="' +
        pid +
        '">' +
        pid +
        "</span></span><br>";
    ...
    $(tr)
      ...
      .click(function () {
        ...
        confirmBox(
          'Room "<b>' +
            roomid +
            '</b>" will be deleted from server. It will disconnect and remove its participants as well.',
          function (isConfirmed) {
            if (!isConfirmed) return;
...
$("#view-logs").click(function () {
  ...
  req.onload = function () {
    ...
    if (keys.length) {
      $("#logs-viewer").html(
        '<div style="text-align: center; margin-bottom: 10px;"><button id="clear-logs" class="btn btn-primary" style="padding: 4px 8px;font-size: 13px;line-height: 1;">Clear Error Logs</button> (permanently delete from logs.json)</div>'
      );
      $("#logs-viewer").append(table);
    } else {
      $("#logs-viewer").html(
        '<div style="text-align: center; margin-bottom: 10px;">Error logs file is empty.</div>'
      );
    }
    ...
    if (keys.length) {
      $("#clear-logs").click(function () {
        $("#logs-viewer").html(
          '<div style="text-align: center; margin-bottom: 10px;">Deleting...</div>'
        );
        socket.emit(
          "admin",
          {
          ...
          function (isCleared) {
            if (isCleared) {
              $("#logs-viewer").html(
                '<div style="text-align: center; margin-bottom: 10px;">Error logs are cleared from logs.json.</div>'
              );
              updateViewLogsButton();
            } else {
...
function getUserInfo(userid, callback) {
  socket.emit(
    ...
    function (userinfo) {
      ...
      var html = "";
      html +=
        '<tr><td>userid</td><td><button data-userid="' +
        userid +
        '" class="btn delete-user" style="float: right;margin-top: -5px;">Delete</button>' +
        userid +
        "</td></tr>";
      [
        "sessionid",
